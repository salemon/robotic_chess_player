...
class ChessboardPoseEstimation():
    ...
    @staticmethod
    def __corner_coordinate(corner,l = 4.3):
    ...
    def detect_chessboard(self,image):
        corner,image_points = self.feature_extractor.general_pose_estimation_points(image)
        points_coordinate = self.__corner_coordinate(corner)
        _, rotation_vector, translation_vector = cv2.solvePnP(
                points_coordinate, image_points, self.camera_matrix,self.dist_coeff)
        ...
        def draw(img, imgpts):
            ...
            return img
        axis = np.float32([[0,0,0],[0.043,0,0], [0,0.043,0], [0.0,0.0,-0.043]]).reshape(-1,3)
        ...
        return Trans3D.from_angaxis(rotation_vector, tvec=translation_vector)
    def __board_coordinate(l = 0.043):
        coordinate = []
        for x in range(1,8):
            for y in range(1,8):
                coordinate.append([x * 0.043, y * 0.043, 0])
        return np.array(coordinate)

    def locate_chessboard(self,image):
        features = self.feature_extractor.closer_pose_estimation_points(image)
        points_coordinate = self.__board_coordinate()
        _, rotation_vector, translation_vector = cv2.solvePnP(
                points_coordinate, features, self.camera_matrix,self.dist_coeff)
        def draw(img, imgpts):
            corner = tuple(imgpts[0].ravel())
            img = cv2.line(img, corner, tuple(imgpts[1].ravel()), (255,0,0), 5)
            img = cv2.line(img, corner, tuple(imgpts[2].ravel()), (0,255,0), 5)
    ...
    @staticmethod
    def square_pixel(alf,num,rot_vect,trans_vect,camera_matrix):
        def constant(value):
    ...
    def estimateSquare(self,image):
        rot_vect,trans_vect = self.locate_chessboard(image)
        '''
        ...
        for world,alf in alf_dict.items():
            for num in range(1,9):
                key = world + str(num)
                square, square_d = self.square_pixel(alf,num,rot_vect,trans_vect,self.camera_matrix)
        ...
        return 'yes',camera2chessboard_pose, 'yes'
...
if __name__ == "__main__":
    ...
    pose = robot.locate_chessboard(image)
    print(pose.to_tfmatrix())
