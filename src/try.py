...
class VisionDetector:
    ...
    def closer_view_pose(self, base2TCP_pose):
        ...
        closer_view_pose = Trans3D.from_tvec(np.array([4*0.043, 4*0.043, -z]))
        return base2TCP_pose * self.TCP2camera_pose * camera2chessbaord_pose * closer_view_pose
    ...
    def locate_chessboard(self,base2TCP_pose):
        self.camera.trigger_image()
        camera2chessboard_pose = self.pose_estimator.detect_chessboard(self.camera.lastest_img)
        pose = base2TCP_pose * self.TCP2camera_pose * camera2chessboard_pose
        base2chessboard_pose = pose * Trans3D.from_tvec(np.array([0,0,-0.1338]))
        #get the tip coordinate of chessboard 
        z = (self.cam_mtx[1][1] / (1 - self.cam_mtx[1][2]) * (-4 * 0.043) + 
        self.cam_mtx[0][0] / (200 - self.cam_mtx[0][2]) * (-4 * 0.043)) / 2
        inv_TCP2camera_pose = Trans3D.from_tfmatrix(inv(self.TCP2camera_pose.to_tfmatrix()))

        return base2TCP_pose * self.TCP2camera_pose * camera2chessboard_pose * Trans3D.from_tvec(np.array([0,0,-0.1338]))
    
    def state_detection_pose(self, base2chessboard_pose):
        inv_TCP2camera_pose = Trans3D.from_tfmatrix(inv(self.TCP2camera_pose.to_tfmatrix()))
...
if __name__ == "__main__":
    ...
    _,points = vision.pose_estimation_points(img)
